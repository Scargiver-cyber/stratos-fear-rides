...
def run_simulation(director_name, quick=False):
    ...
    # ------------------------------------------------------------------
    # OPTIONAL CREW REASSIGNMENT PHASE
    # ------------------------------------------------------------------
    if ask_yes_no("Would you like to review and reassign crew between spacecraft?"):
        while True:
            p("\n--- CREW REASSIGNMENT ---")
            # Show a quick summary of crew counts per ship
            for idx, craft in enumerate(spacecraft_fleet, start=1):
                crew_count = sum(1 for c in craft.crew.values() if c is not None)
                p(f"{idx}. {craft.name}: {crew_count}/4 crew assigned")
            p("0. Done reassigning crew")

            ship_choice = ask_int(
                "Select a SOURCE ship to move crew from (or 0 to finish)",
                min_value=0,
                max_value=len(spacecraft_fleet),
            )
            if ship_choice == 0:
            ...
            source = spacecraft_fleet[ship_choice - 1]
            # Build a list of roles that currently have crew assigned
            roles_with_members = [r for r, m in source.crew.items() if m is not None]
            if not roles_with_members:
                p("  [INFO]", source.name, "has no crew to move.")
                continue

            p("\nSelect which crew role to move from", source.name)
            for idx, role in enumerate(roles_with_members, start=1):
                member = source.crew[role]
                p(f"  {idx}. {role.capitalize()} - {member.name} (XP {member.experience})")
            role_choice = ask_int(
                "Choose a role to move",
                min_value=1,
                max_value=len(roles_with_members),
            )
            role_to_move = roles_with_members[role_choice - 1]

            # Pick target ship
            target_choice = ask_int(
                "Select a TARGET ship to receive this crew member",
                min_value=1,
                max_value=len(spacecraft_fleet),
            )
            target = spacecraft_fleet[target_choice - 1]

            if target is source:
                p("  [INFO] Source and target ship are the same; no reassignment made.")
                continue

            if target.crew.get(role_to_move) is not None:
                p("  [ERROR]", target.name, "already has a", role_to_move, "assigned.")
                continue

            # Perform the move
            moved_member = source.unassign_crew_member(role_to_move)
            if moved_member is None:
                continue
            target.assign_crew_member(moved_member)
            p("  [CREW]", moved_member.name, "moved from", source.name, "to", target.name)
    # ------------------------------------------------------------------
    # MISSIONS
    # ------------------------------------------------------------------
    p("\n--- LOADING MISSIONS ---")
